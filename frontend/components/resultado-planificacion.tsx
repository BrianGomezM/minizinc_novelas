...
export function ResultadoPlanificacion({ solucion, data }) {
  ...
  const costoPromedioPorActor =
    solucion.costosActores.reduce((sum, costo) => sum + costo.costoTotal, 0) / solucion.costosActores.length

  const actorMasCostoso = solucion.costosActores.reduce(
    (max, costo) => (costo.costoTotal > max.costoTotal ? costo : max),
    { costoTotal: 0 },
  )

  const nombreActorMasCostoso = data.actores.find((a) => a.id === actorMasCostoso.actorId)?.nombre || "Desconocido"
              ...
              <TabsContent value="orden">
                <div className="bg-white p-3 sm:p-4 rounded-lg border border-gray-300">
                  ...
                  <div className="flex flex-wrap gap-2 mb-4 overflow-x-auto pb-2">
                    {solucion.ordenEscenas.map((escenaId, index) => {
                      const escena = data.escenas.find((e) => e.id === escenaId)
                      return (
              ...
              <TabsContent value="grafico">
                <div className="bg-white p-3 sm:p-4 rounded-lg border border-gray-300">
                  <h3 className="text-lg font-semibold mb-4 text-gray-800">
                    Visualización de Participación en el Tiempo
                  </h3>
                  <GraficoParticipacion solucion={solucion} data={data} />
                </div>
              </TabsContent>
            </Tabs>
          </div>
